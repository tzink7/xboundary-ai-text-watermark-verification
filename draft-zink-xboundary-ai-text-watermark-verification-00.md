...
* p  = REQUIRED, unless "k=symmetric" is present (see "k=" below). Public key material, base64-encoded. This value is only safe to publish for an asymmetric, publicly-detectable watermarking scheme (Section 4.1); publishing a symmetric scheme's secret key under this tag exposes the ability to forge and strip the watermark, not merely verify it. See Section 10.3.
 ...
 * nb = REQUIRED. "Not before" -- the start of this key's validity window, expressed as a Unix timestamp (seconds since the epoch). Unlike "s=" above, this tag has no safe default: a verifier checks whether the current time, at verification, falls within the nb=/na= window (Section 9.2), and an absent "nb=" would leave that window open-ended on one side, defeating the time-bounded check the same way an absent "c=" would silently misrepresent custody.
...
* **Double-signing: first with a symmetric or asymmetric algorithm, and then second with a zero-width algorithm.** One case for future expansion is to encode multiple watermarks: a symmetric or asymmetric algorithm embeds a statistical mark into the text, and a zero-width character layer on top of that encodes the DNS record locator and any other required metadata. That zero-width layer could carry more than just its own locator: it could also name the selector for the statistical mark underneath it, telling a verifier to check that one too -- or, for a symmetric scheme (which has no DNS-published key to look up), the URL of the provider's own verification API or web page instead. A verifier would then be told directly how to retrieve both sets of verification material (two direct DNS lookups, or a local cache lookup) rather than having to iterate over multiple keys. A malformed, partially-stripped, or malicious zero-width layer may carry an incorrect locator; the fallback in that case is simply to verify as if the watermark carried no locator information at all.
* **Custom selectors.** If a double-signature is possible, and embedding the record locator is possible in the zero-width algorithm, then a custom selector (i.e., `customer-selector._watermark-text.example.ai`) is also possible. In this case, a verifier would first try the DNS record that is explicitly specified as a means of improving verification performance. If it did not work, a verifier would fallback to standard verification (i.e., trying all its keys, or applying some other heuristic as the most recently used keys).
