...
2. Terminology
3. Problem Statement 
  * 3.1.  Vendor-Siloed Detection 
  * 3.2.  The Laundering Vulnerability 
  * 3.3.  Absence of Multi-Hop Lineage
4. Relationship to Existing Work 
  * 4.1.  Statistical Watermarking (SynthID and Similar Schemes) 
  * 4.2.  C2PA and Content Credentials 
  * 4.3.  Why Neither Solves Cross-Vendor Text Verification
  * 4.4.  Challenges with leveraging APIs exposed by providers
5. Architecture Overview
6. DNS Key Distribution 
  * 6.1.  Record Location and Syntax 
  * 6.2.  Key Rotation and Multiple Records 
  * 6.3.  Caching and Refresh Behavior 
  * 6.4.  Provider Discovery and Bootstrap Procedure 
  * 6.5.  Delegation
7. Multi-Hop Attestation 
  * 7.1.  Cross-Vendor Handoff 
  * 7.2.  JSON Custody Descriptor Format (d=) 
  * 7.3.  Intra-Model (Self-Loop) Re-signing
  * 7.4.  Composite Key Records for Chained Custody
  * 7.5.  Worked Examples
8. The Laundering Tax
9. Security Considerations 
  * 9.1.  DNS Integrity and DNSSEC Dependence
  * 9.2.  Key Lifecycle, Compromise, and Rotation 
  * 9.3.  Replay Attacks and Context Stripping
  * 9.4.  Custody Descriptor Tampering `(dh=)`
  * 9.5.  Non-Repudiation and Semantic Truth Boundaries
  * 9.6.  Non-Adoption and the Open-Weights Gap.
10. Known Limitations 
  * 10.1. Signal Capacity of Statistical Watermarks 
  * 10.2. This Framework Does Not Create Detectability 
  * 10.3. This Framework Requires a Cryptographic Primitive Not Yet Deployed at Scale
11. Incentive Analysis
12. Performance Considerations
13. Future specification expansion
14. Open Questions for Review
15. IANA Considerations
16. References 
17. Appendix A.  Design Note: Avoiding DKIM Tag-Letter Collisions
18. Appendix B.  Design Alternatives Considered: Monolithic Key JSON vs. Per-Selector DNS Records
...
The record value follows a tag-value syntax deliberately similar to DKIM (RFC 6376) and BIMI:
...
* a  = REQUIRED. Algorithm identifier, naming the watermarking scheme this key applies to (e.g., "fairoze"). See below and Section 15 for the registry this tag depends on.
...
When Model B receives text already bearing Model A's watermark and modifies it, Model B SHOULD NOT simply overwrite Model A's signal with its own. Instead, following an ARC-like pattern (RFC 8617), Model B appends an attestation identifying the handoff: which provider preceded it. This produces an ordered, inspectable custody chain rather than a single overwritten mark that erases prior history.
...
### 7.2.1.  Example Custody Document
...
```{
  ...
  "selector": REQUIRED. Corresponds to the selector of the signing provider's DNS record,
  "provider": REQUIRED. Corresponds to the domain of the signing provider,
  "c": REQUIRED. Range of values are "sign" or "re-sign",  
  "ts": REQUIRED. Corresponds to a UNIX epoch time stamp of when the record was published.  
}
```
...
### 7.2.2.  Verification Mechanics (d=, dh=, and c=)
...
* under a key whose nb=/na= window covers the approximate time the text appears to have been produced, and 
* whose "s=" status is "active" (or was, for that window). No custody chain is involved -- this is a single, unmodified hop.
...
* **DNSSEC Enforcement:** Domain owners publishing _watermark-text TXT records SHOULD implement DNSSEC (RFC 4033). Verifying resolvers MUST perform DNSSEC validation where available to guarantee record authenticity and origin integrity.
...
* **Validity Windows** `(nb= / na=)`: Public key TXT records MUST include validity parameters (nb= for Not Before, na= for Not After). Verifiers MUST evaluate text generation timestamps against these windows and reject signatures evaluated outside an active key's operational lifespan.

* **Revocation Status** `(s=)`: If a private key is compromised, the publishing domain MUST immediately update the selector record status tag to s=revoked (or s=deprecated). Verifiers MUST treat records marked with s=revoked as invalid for all verification attempts, including historical text.
...
### 11.2.  Potential use case - transparency in social media feeds
...
### 11.3.  Potential use case - transparency in community-sourced knowledge bases
...
### 11.4.  Potential use case - as contrast from other types of text
...
* No integrity mechanism is defined for "d=" custody-meaning documents beyond HTTPS transport security, meaning their content can change after publication without detection (Section 9.3).
...
[BIMI]     Brand Indicators for Message Identification (BIMI), Authentication and Email Security Working Group, M3AAWG / AuthIndicators Working Group specification.

[C2PA]     Coalition for Content Provenance and Authenticity, Technical Specification.

[SynthID]  Dathathri, S., et al., "Scalable watermarking for identifying large language model outputs," Nature, vol. 634, October 2024.

[Fairoze23] Fairoze, J., Garg, S., Jha, S., Mahloujifar, S., Mahmoody, M., and Wang, M., "Publicly-Detectable Watermarking for Language Models," IACR ePrint 2023/1661, 2023. Describes an asymmetric, publicly-detectable watermarking scheme in which the detection algorithm contains no secret information, in contrast to the symmetric schemes discussed in Section 4.1.

[Gloaguen24] Gloaguen, T., Jovanovic, N., Staab, R., and Vechev, M., "Black-Box Detection of Language Model Watermarks," arXiv:2405.20777, 2024. Source for the claim in Section 4.1 that a production watermarking system, despite introducing novel components such as tournament sampling, still falls within the "Red- Green" family of watermarking schemes under the paper's detection-theoretic tests -- evidence that distinct deployed schemes share underlying structure.

Additional informative references to industry and press coverage of provider watermarking deployments (August 2026) to be added in the next revision with full citations.
...
### B.1. Evaluated Alternative Architecture
...
### B.2. The dh= Invalidation Cascade (The DNS Update Paradox)
...
### B.3. Destruction of Granular DNS Delegation (CNAMEs)
...
### B.4. Shift from Resilient DNS Edge to Vulnerable HTTP Origins
...
### B.5. Payload Bloat and Bandwidth Inefficiency
