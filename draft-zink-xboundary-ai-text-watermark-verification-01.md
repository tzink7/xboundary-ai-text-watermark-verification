...
[seriesInfo]
name = "Internet-Draft"
value = "draft-zink-xboundary-ai-text-watermark-verification-01"
stream = "IETF"
status = "standard"
...
This document's "p=" key-distribution mechanism (Section 6) is therefore only sound if applied to an asymmetric, publicly-detectable watermarking scheme. It is NOT sound if applied naively to a currently-deployed symmetric scheme: doing so would not merely enable third-party verification, it would hand out the ability to forge and strip the watermark to anyone who queries the DNS record. This is a significant, unresolved gap between what this document assumes and what labs have actually shipped as of this writing; see Section 11.3.

Regulation does not obviously close this gap by pushing providers toward asymmetric schemes; if anything it pushes the other way. The EU's General-Purpose AI Code of Practice, published by the European Commission alongside the AI Act's Article 50 transparency obligations, is understood to require that AI-generated content carry a marking embedded in the content and not trivially removable, and -- where a provider relies on a single marking mechanism -- that the mechanism be robust. A deployed symmetric statistical mark already meets "embedded" and "reasonably robust"; a current asymmetric scheme is less mature and, by most accounts, less robust to editing. A provider told to keep one robust embedded mark therefore has a reason NOT to migrate. This document accommodates that reality in two bounded ways: "k=symmetric" (Section 6.1) with the verification document of Section 6.6 standardizes *discovery* of a symmetric provider's own verification service (without making a bare symmetric mark publicly forge-resistant); and Section 8 (Composed Marks) specifies a construction that keeps the robust inner mark and adds a signed, self-describing outer layer for the verification path. Neither turns a symmetric mark into an asymmetric one.

This proposal does not invent a watermark embedding algorithm. It assumes providers adopt (or migrate to) an asymmetric, publicly-detectable embedding scheme for genuine cross-organization verification, and addresses only key distribution, multi-hop attestation, and mark composition around such a scheme once adopted.
...
This creates a DoS problem, particularly if a scripted, adversarial service attempts to attack a Provider. This would create an escalating arms race between Providers and malicious abusers just to answer the question “Was this text generated by AI?”

The use of cross-provider APIs does not scale as the amount of usage grows.

# Architecture Overview

The framework has four components, and assumes a fifth (represented below as 0):
...
3. A composed-mark construction (Section 8), allowing a provider to keep a robust inner mark -- including a deployed symmetric one -- and add a signed, self-describing outer manifest that names where each mark is verified.

4. An explicit acknowledgment (Section 9) that heavily laundered text is expected to degrade in quality as a natural consequence of compounding constraints, and a discussion of why that is treated as an acceptable outcome rather than a flaw to be engineered away.

5. Verification key material and hop metadata are the objects this framework standardizes. It does not standardize, and takes no position on, the internal embedding algorithm any given provider uses.
...
* a  = REQUIRED. Algorithm identifier, naming the watermarking scheme this key applies to (e.g., "tzsataitw", "fairoze"). See below and Section 16 for the registry this tag depends on.
* p  = REQUIRED, unless "k=symmetric" is present (see "k=" below). Public key material, base64-encoded. This value is only safe to publish for an asymmetric, publicly-detectable watermarking scheme (Section 4.1); publishing a symmetric scheme's secret key under this tag exposes the ability to forge and strip the watermark, not merely verify it. See Section 11.3.
* k  = OPTIONAL. Key model for the "p=" value. The only value this document defines is "symmetric", which asserts that the watermarking scheme named in "a=" has no public verification key. When "k=symmetric" is present, "p=" MUST be absent and "d=" MUST be present, carrying the verification document described under "d=" above: verification for a symmetric scheme can only be performed by the key holder, and that document is where the out-of-band verification endpoint is named. When "k=" is absent the scheme is asymmetric and "p=" is REQUIRED. The three well-formed combinations a verifier may therefore see are: "p=" present with "k=" absent (asymmetric); "k=symmetric" with "p=" absent (symmetric); and nothing else -- "p=" and "k=" both absent, or "k=symmetric" alongside a "p=", is a malformed record and a verifier SHOULD treat it as unusable. This value is deliberately not carried by omitting "p=" alone (which would be indistinguishable from a publishing error) nor as an empty "p=;" (which in DKIM key records means a revoked key -- a collision Appendix A exists to avoid). A future revision MAY define "k=" values naming specific asymmetric key types (e.g. "ed25519") as an optional, must-be-consistent hint; existing asymmetric records need not be updated for that.
...
* d  = an HTTPS URL (HTTPS is REQUIRED; plain HTTP MUST NOT be used) pointing to a machine-readable (JSON) document carrying detail about this selector that does not fit the DNS record itself. The JSON form is normative and MUST be available; a human-readable rendering of the same content MAY also be served at the same URL via content negotiation. The document defines one or both of two things, and which a verifier expects is fixed by the other tags in the record: (a) for a cross-vendor "c=re-sign", the **custody descriptor** of Section 7.2 -- REQUIRED in that case; (b) for a "k=symmetric" scheme, the **verification document** of Section 6.6 -- REQUIRED in that case -- which names a "verify" endpoint and the canonicalization a verifier applies before calling it, with any scheme-specific fields added by the "a=" registration (Section 16). A record that is both a cross-vendor re-sign and symmetric carries a document with both sets of fields. "d=" is OPTIONAL otherwise: an ordinary "c=sign" asymmetric record, or a "c=re-sign" same-provider self-loop, with no further detail to convey beyond the record's own tags.
...
Unlike the tags above, which apply uniformly to every selector's record, one further tag is scoped to a single record only:
...
Step 1's bootstrap-trust gap is worth stating plainly rather than glossing over: nothing in this framework cryptographically proves that a given domain belongs to a legitimate AI provider, or that a seed list has not been tampered with or omitted a participant. This document does not propose a solution and is listed as an open question in Section 15.
...
The document MUST be a UTF-8 JSON object. Five fields are common to every verification document, whatever the algorithm; the first three are REQUIRED, the last two OPTIONAL:
...
* "verify" -- REQUIRED. An HTTPS URL (HTTPS REQUIRED; plain HTTP MUST NOT be used) where the check is performed. This document does NOT mandate that it be a machine API. It MAY be a programmatic endpoint, or a human-operated web page a person pastes text into and reads a result from. Both are legitimate: the point of "k=symmetric" is to standardize *discovery* of the provider's verification service, not its form.
  When "verify" is a programmatic endpoint, it SHOULD accept an HTTP POST with "Content-Type: text/plain; charset=utf-8" and the canonicalized text as the request body, and return a JSON object with at least `{"algorithm": <string>, "watermarked": <boolean>}`, and SHOULD also return "score" and "threshold" numbers so a verifier can present confidence rather than a bare yes/no. The endpoint MAY rate-limit; a verifier MUST treat HTTP 429 or 503 as "could not verify", never as a negative result. Access MAY require credentials the verifier obtains out of band (see "access" below); an endpoint that requires credentials SHOULD answer an unauthenticated request with HTTP 401 or 403 rather than a misleading negative result.
...
* "access" -- OPTIONAL. An HTTPS URL to a page describing how a party obtains access to the "verify" endpoint -- registration, credential issuance, terms, rate tiers. A provider whose "verify" endpoint is not openly usable SHOULD publish this so a verifier is not left guessing. Its absence means the provider asserts "verify" is usable without prior arrangement.
...
An "a=" registration (Section 16) MAY define further algorithm-specific fields -- for a statistical scheme, the tokenizer identifier, context parameters, and detection threshold that let a party already holding the secret key reproduce detection locally. These are OPTIONAL to publish: a verifier without the key cannot use them, a provider MAY prefer to keep the detector's internal structure private, and when present they are informational, carrying no normative weight for the "verify"-endpoint flow.

The verification document is fetched under the same rules as the custody document (Section 7.2): redirects are followed with the HTTPS and public-IP checks re-applied on every hop, and when the DNS record carries "dh=", the digest is computed over the final response body and MUST match.

Example, for a "synthid-1" record:
...
In this case, rather than attempting to verify a mark itself -- which would not be possible, since synthid-1 is a symmetric scheme and the verifier does not hold the key -- a verifier refers to https://example.ai/watermark/verify, either programmatically or by directing the user to open it and paste the text.

This is deliberately the provider-run-service pattern that Section 4.4 argues does not scale to Internet volumes, and "k=symmetric" does not solve that: a symmetric scheme structurally cannot be verified by anyone but the key holder, so a provider that ships only a symmetric mark leaves a third party no option but to call the provider's service (with the latency, credential, and denial-of-service costs Section 4.4 describes) or hand the task to a human. What this section standardizes is only the *discovery* of that service -- a verifier learns where to send the text from a DNS lookup and a small JSON document, rather than needing a prior integration with each provider. Providers who want genuine, scalable cross-organization verification still need an asymmetric scheme (Section 4.1), or a composed mark whose outer layer is asymmetric (Section 8).
...
The precise wire format for composite/chained attestations (analogous to ARC's AAR/AS/AMS header set) is left as an open design question for this draft; see Section 15.
...
* under a key whose nb=/na= window covers the current time, at the moment the verifier is checking (Section 10.2), and 
...
Section 15 lists this as an open question in the case where two providers' claims about the same handoff disagree.
...
# Composed Marks (Double Signatures)

## Motivation

The core mechanism of this document (Section 6) is sound only for an asymmetric, publicly-detectable watermarking scheme (Section 4.1). The schemes actually deployed at production scale are symmetric, and regulation is not obviously moving providers off them. The EU's General-Purpose AI Code of Practice, published by the European Commission alongside the AI Act's Article 50 transparency obligations, is understood to require that AI-generated content carry a marking that is embedded in the content itself and not trivially removable, and -- where a provider relies on a single marking mechanism -- to hold that mechanism to being robust. A deployed symmetric statistical mark already satisfies "embedded" and "reasonably robust"; what it lacks is cross-organization verifiability. Migrating to a less mature asymmetric scheme to gain that verifiability could mean giving up robustness, which a single-mechanism provider may not be permitted to do.

A *composed mark* addresses this without that trade. The provider keeps its embedded, robust statistical mark -- symmetric or asymmetric -- as the inner layer, and adds a second, outer mark: a lightweight steganographic layer (Section 6.1, a "tzsataitw"-class algorithm) carrying an Ed25519-**signed manifest** that names the inner mark and where to verify it. This is a *double signature*: one party, at one hop, deliberately applying two marks. It is not a custody handoff (Section 7) and does not use "c=re-sign".

The outer layer is not robust -- any Unicode normalization removes it -- so a composed mark is not a way to make a symmetric mark durably forge-resistant. It is a fast, self-describing verification path that works while the outer layer survives and degrades cleanly to "verify the inner mark with no locator information" when it does not (Section 8.6).

## The Composed Artifact

A composed mark is one piece of text carrying:

* an **inner mark** -- a statistical watermark (symmetric or asymmetric) embedded at generation, imperceptible, carried in the token-selection distribution; and
* an **outer mark** -- a steganographic layer in a channel that does not alter the word sequence (this document's reference channel is zero-width characters inserted between words), carrying the signed manifest.

The outer channel MUST be one whose removal is a pure character-level normalization that leaves the inner statistical mark intact. Zero-width characters satisfy this. A channel that substitutes visible characters (for example, homoglyph letters) does not: stripping it changes code points that a character-sensitive inner detector would otherwise see.

## The Manifest

The outer mark's payload is a *manifest*: an ordered list of the marks on the text, plus the location of the key that signed the manifest.

```
manifest = mver | n_marks
           n_marks * ( scheme-id | locator )
           sig-locator
           signature
```

* **mver** -- manifest format version; this document defines version 1.
* **scheme-id** -- the "a=" identifier (Section 6.1, Section 16) of one mark present on the text.
* **locator** -- the "<n>._watermark-text.<domain>" record for that mark. For an asymmetric inner mark this is where its "p=" is published; for a symmetric inner mark ("k=symmetric") it is the record whose "d=" points to the Section 6.6 verification document.
* **sig-locator** -- the "<n>._watermark-text.<domain>" record whose "p=" verifies the signature below. Its "a=" MUST name a steganographic ("tzsataitw"-class) algorithm and its "c=" MUST be "sign".
* **signature** -- a signature (Ed25519 for the reference channel; other schemes MAY be registered) over

  `"<outer scheme-id>/manifest/v<mver>\n" || <the manifest bytes above the signature> || 0x00 || canonical-text`

  where `canonical-text` is the text with every mark's channel undone per the outer algorithm's canonicalization (Section 6.1: zero-width characters removed, look-alikes folded, NFC, leading/trailing whitespace trimmed). Because `canonical-text` still carries the inner statistical bias, the signature binds the two layers: an edit that washes out the inner mark also changes `canonical-text` and breaks the outer signature.

The concrete byte layout of the manifest is defined by the outer algorithm's registration (Section 16), the same way each "a=" registration defines its own frame. This section defines only the fields, and the signed-message construction that every manifest format MUST use.

## DNS Records

A composed mark involves two records, both published by the provider that applied it:

* the **outer record** -- at `sig-locator`: "a=<tzsataitw-class>", "c=sign", "p=<public key>". A provider MAY reuse an existing "sign" selector; nothing distinguishes a selector that signs manifests from one that signs bare marks, because a manifest is self-identifying once extracted.
* the **inner record** -- unchanged from its stand-alone form: asymmetric ("p="), or symmetric ("k=symmetric" with "d=" pointing to a Section 6.6 document).

A composed mark that a single provider applies to its own generated text does **not** require a Section 7.2 custody descriptor: the signed manifest is itself the record of what was done, cryptographically bound rather than hash-pinned at a URL. When the inner and outer marks are from *different* providers, the outer act is a cross-vendor re-sign and Section 7.1 applies in addition to this section.

## Verification

A verifier that recovers an outer manifest frame from the text MUST proceed in this order:

1. **Parse** the manifest: the marks list, `sig-locator`, and `signature`.
2. **Verify the outer signature.** Compute `canonical-text` and the signed message above. Resolve `sig-locator` via DNS (Section 6.4), confirm its "a=" names a steganographic algorithm, and verify `signature` against its "p=".
3. **If step 2 fails**, the manifest is not authenticated and the verifier MUST NOT act on the records it names -- a forged manifest can point anywhere. The verifier falls back to verifying the text as if it carried no locator information: strip the outer channel and attempt each inner scheme against the verifier's own key cache and/or a provider domain the user supplies (Section 6.4, step 5).
4. **If step 2 succeeds**, the manifest is authentic. For each entry in the marks list, the verifier strips the outer channel, resolves the named record, confirms the record's "a=" equals the entry's `scheme-id`, and verifies that mark under its own scheme's rules: an asymmetric scheme against the record's "p="; a symmetric scheme by the Section 6.6 flow (fetch "d=", apply its canonicalization, submit to its "verify" endpoint).
5. **The overall result is the composition.** The outer signature identifies the signing provider and vouches for the manifest; each inner result is reported on its own terms -- a signature verdict, or a statistical score against a threshold. A verifier SHOULD present all of them rather than collapse them to a single yes/no.

The inner scheme in step 4 runs on the outer-channel-stripped text, canonicalized by that scheme's own rules -- it is not re-canonicalized by the outer algorithm. Section 6.6's canonicalization token set is a superset of the outer strip and is idempotent with respect to it, so no conflict arises for the reference channels.

## The Manifest Is an Optimization, Not a Guarantee

The outer channel is removed by the same trivial normalization that defeats any steganographic mark. A normalized copy of a composed mark loses the entire manifest and is indistinguishable from the bare inner mark. Everything a verifier can do with a manifest it can also do without one, given the provider's domain: the manifest removes the "which provider, which selector" search, it does not add robustness to the inner mark. Implementations MUST treat a missing or unverifiable manifest as a routine case (step 3 above), not an error.
...
This is explicitly not a claim that laundering can be made impossible, nor that all adversaries are quality-sensitive. See Section 11 and Section 12 for the limits of this argument.
...
* **Double-signing** -- a statistical inner mark plus a signed, self-describing outer manifest -- was previously listed here as future work. It is now specified in Section 8 (Composed Marks). What remains open is the manifest's concrete byte layout, which Section 8.3 defers to the outer algorithm's registration (Section 16): no registration yet defines one normatively.
* **Custom selectors.** Given a composed mark (Section 8), whose outer manifest can carry an arbitrary locator, a custom selector (e.g. `customer-name._watermark-text.example.ai`) becomes possible: a verifier would first try the record the manifest names as a performance shortcut, and fall back to standard verification (trying its cached keys, or a most-recently-used heuristic) if that record does not resolve or does not verify.
...
* Payload budget for in-band custody metadata (Section 11.1) has not been quantified against any specific provider's actual watermarking scheme.
* No proposal is made here for how a verifier should weight or resolve conflicting attestations from different providers.
* Establishment and stewardship of the "a=" algorithm registry (Section 16) is unresolved, including who is positioned to register algorithm identifiers for schemes that already exist but were not designed with this framework in mind.
...
Establishing this registry, and determining who is positioned to act as registrant for schemes already deployed by third parties (e.g., SynthID), is left as an open question for further discussion; see Section 15.
...
However, to prevent retroactive tampering or silent history alteration of the hosted JSON document (Section 10.4), the DNS record MUST include a Subresource Integrity digest tag (dh=).
