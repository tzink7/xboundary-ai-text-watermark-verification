...
| File | What it is |
|---|---|
| `draft-zink-xboundary-ai-text-watermark-verification-01.md` | The specification: problem statement, architecture, DNS record syntax, multi-hop attestation, symmetric-scheme accommodation (`k=symmetric` verification documents, composed marks), worked examples, security considerations, known limitations, and open questions |
| `README.md` | This file |
...
| `tools/tzsataitw.py` | An asymmetric text watermark (zero-width-character and homoglyph channels). Steganographic and short-text-capable, but trivially removed by text normalization. `--co-sign` adds a signed manifest over another mark for a §8 composed mark |
...
| `tools/synthid.py` | Verifier for `synthid-1` — SynthID-Text [Dathathri24], the *symmetric* comparison point. Masked-mean detector; `--calibrate` sets a threshold from a non-watermarked corpus. Needs `torch` + `transformers` (`tools/requirements-synthid.txt`) |
| `tools/synthid_profile.py`, `tools/synthid_smoke.py`, `tools/synthid_score_samples.py` | The `synthid-1` contract, the batch sample generator, and the script that (re)builds `samples/synthid-1/verify-scores.json` |
...
| `tests/` | Test suites for the `fairoze-1` verifier, the `synthid-1` verifier, and the `tzsataitw` double-signature manifest (`python tests/test_*.py`) |
| `samples/fairoze-1/`, `samples/synthid-1/` | Real watermarked passages for each scheme, plus keys / records / calibration data they verify against |
...
| `demo/` | A live web sandbox for the tools above (watermark, verify, build a record, validate a domain) — [watermark.demo.terryzink.com](https://watermark.demo.terryzink.com), running on Google Cloud Run. `demo/DEPLOY.md` covers redeploying it |
...
- **§11.3 / §15** — The whole mechanism depends on an asymmetric watermarking scheme that isn't running in production yet. Deployed schemes (including Anthropic's SynthID-Text-based implementation) are symmetric.
- **§6.4 / §15** — Nothing here cryptographically proves a domain belongs to a legitimate provider. The seed-list bootstrap-trust problem is inherited, not solved.
- **§10.5 / §7.5(d)** — Multi-hop custody claims are self-attested. A verifier can confirm who signed a handoff, not whether the claim about where the text came from is true.
- **§15** — No revocation-propagation mechanism, no proposal for resolving conflicting custody claims between providers, and no treatment of the statistical effect of testing text against an ever-larger set of cached keys.

The full list is in [Section 15](./draft-zink-xboundary-ai-text-watermark-verification-01.md) of the draft, and each item is a candidate for its own GitHub Issue — see CONTRIBUTING.md.
